    ...
    <div className="min-h-screen pt-[80px] bg-[var(--paper)]">
      <div className="max-w-[800px] mx-auto px-6 py-8">
        <div className="flex justify-between items-center mb-7">
          <div>
            <h1 className="font-display text-[32px] font-bold text-[var(--ink)] tracking-tight">Artikel Saya</h1>
            <p className="text-[14px] text-[var(--ink-lt)] mt-1">Semua artikel yang pernah Anda tulis</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white border border-[rgba(28,43,43,0.10)] rounded-xl">
            ...
            <p className="text-[13px] text-[var(--ink-lt)] mb-5">Mulai tulis artikel pertama Anda sekarang!</p>
            <Link href="/editor/new"
          ...
          <div className="flex flex-col gap-3">
            {articles.map(a => {
              ...
              return (
                <div key={a.id} className="bg-white border border-[rgba(28,43,43,0.10)] rounded-xl p-5 flex gap-4 items-start hover:border-[rgba(28,43,43,0.18)] transition-colors">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-[15px] text-[var(--ink)] mb-1.5 line-clamp-2">{a.judul || '(Tanpa judul)'}</h3>
                    {a.abstrak && <p className="text-[13px] text-[var(--ink-lt)] line-clamp-1 mb-2">{a.abstrak}</p>}
                    <div className="flex items-center gap-3 flex-wrap">
                      <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold ${st.cls}`}>{st.label}</span>
                      <span className="text-[12px] text-[var(--ink-lt)]">📅 {formatDate(a.updated_at)}</span>
                      {a.status === 'rejected' && a.catatan_review && (
                        <span className="text-[12px] text-[#C05030]">💬 {a.catatan_review}</span>
                      )}
                    </div>
                  </div>
