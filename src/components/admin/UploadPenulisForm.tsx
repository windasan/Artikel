...
export function UploadPenulisForm({ penulisList }: { penulisList: Profile[] }) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!form.nama_lengkap || !form.nim || !form.email) {
      toast.error('Nama, NIM, dan email wajib diisi')
      return
    }
    setLoading(true)
    ...
    if (existing) {
      ...
      else { toast.success('✅ Data anggota diperbarui!'); setForm(EMPTY); router.refresh() }
    } else {
      toast.error(
        'Pengguna dengan email ini belum pernah login. Minta mereka login dulu via Google.',
        { duration: 6000 }
      )
    }
    setLoading(false)
  ...
  const handleCSV = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const row of rows) {
      ...
      const validRole = Object.keys(ROLE_LABELS).includes(role) ? role : 'design_layout'
      const { error } = await supabase.from('profiles').update({
        nama_lengkap: nama, nim, no_telepon: telp || null, role: validRole,
      }).eq('email', email)
        ...
        {tab === 'form' && (
          <form onSubmit={handleSave}>
            <p className="text-[13px] text-[var(--ink-lt)] mb-5 leading-relaxed">
              Tambah atau perbarui data anggota. Anggota harus sudah pernah login via Google agar akunnya terdaftar.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
              ...
              <div>
                ...
                <p className="text-[10px] text-[var(--ink-lt)] mt-1 leading-relaxed">
                  Design & Layout: upload artikel · Redaksi: review · Publikasi: tag & publish · IT: kelola kelompok
                </p>
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <textarea value={form.bio} onChange={e => set('bio', e.target.value)}
                  placeholder="Deskripsi singkat anggota (opsional)..."
                  rows={2}
        ...
        {tab === 'csv' && (
          <div>
            <p className="text-[13px] text-[var(--ink-lt)] mb-4 leading-relaxed">
              Upload file CSV untuk memperbarui data banyak anggota sekaligus. Format kolom:
            ...
            <div className="bg-[var(--cream)] rounded-xl p-4 mb-5 font-mono text-[12px] text-[var(--ink-md)]">
              <p className="font-bold text-[var(--ink-lt)] mb-1">Contoh CSV (role: admin/design_layout/redaksi/publikasi/it):</p>
              <p>nama_lengkap,nim,email,no_telepon,role</p>
              <p>Rafi Santoso,21812141001,rafi@student.uny.ac.id,08123456789,design_layout</p>
              <p>Dian Permata,21812141002,dian@student.uny.ac.id,08234567890,redaksi</p>
            </div>
        ...
        {tab === 'list' && (
          <div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-[13px]">
                <thead>
                  <tr className="bg-[rgba(28,43,43,0.025)]">
                    {['#', 'Nama', 'NIM', 'Email', 'No. HP', 'Role / Divisi'].map(h => (
                      <th key={h}
                        className="px-3 py-2.5 text-left text-[11px] font-bold uppercase tracking-wider text-[var(--ink-lt)]">
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {filtered.map((p, i) => (
                    <tr key={p.id}
                      ...
                      <td className="px-3 py-3">
                        <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold ${ROLE_COLORS[p.role] ?? ROLE_COLORS.design_layout}`}>
              ...
              {filtered.length === 0 && (
                <div className="text-center py-8 text-[var(--ink-lt)] text-[13px]">
