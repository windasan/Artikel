...
interface EditorState {
  ...
  kategori_id: string; kelompok_id: string; author_type: AuthorType
  volume: string; nomor_edisi: string; penulis_ids: string[]
  ...
  return (
    <div className="border-b border-[rgba(28,43,43,0.07)] pb-5 mb-0">
      <button type="button" onClick={() => setOpen(!open)}
        ...
        {open
          ? <ChevronUp size={13} className="text-[var(--ink-lt)]" />
...
function InputField({ label, ...props }: { label: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <div className="mb-3">
      {label && (
        <label className="block text-[11px] font-semibold text-[var(--ink-lt)] mb-1.5">{label}</label>
      )}
      <input {...props}
...
export function EditorSidebar({
  ...
  const removeKw = (kw: string) =>
    updateState('kata_kunci', state.kata_kunci.filter(k => k !== kw))
  ...
  const handleAuthorTypeChange = (type: AuthorType) => {
    updateState('author_type', type)
    // Reset penulis_ids when switching; EditorContainer's useEffect re-populates for 'group'
    if (type === 'individual') {
      updateState('penulis_ids', [])
    }
  }
    ...
    <div className="w-[300px] flex-shrink-0 bg-white border-l border-[rgba(28,43,43,0.10)] overflow-y-auto h-full">
      <div className="p-5 space-y-5">
          ...
          <div className="bg-[var(--cream)] rounded-xl p-4 mb-3">
            <div className="space-y-2 mb-4 text-[12px]">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono text-[11px] text-[var(--ink-lt)]">
                  {articleId?.slice(0, 8) ?? '—'}
                </span>
              </div>
            </div>
            <button type="button" onClick={onSubmit}
              ...
              Kirim ke Tim Redaksi →
            </button>
            <p className="text-[11px] text-[var(--ink-lt)] text-center leading-relaxed">
              Artikel akan diperiksa Tim Redaksi sebelum ke tahap publikasi
            </p>
          </div>
        </Section>
        ...
        <Section title="Penulis Artikel">
          ...
          <div className="flex rounded-lg border border-[rgba(28,43,43,0.12)] overflow-hidden mb-3">
            <button
              type="button"
              onClick={() => handleAuthorTypeChange('individual')}
              className={`flex-1 py-2 text-[12px] font-semibold transition-colors ${
                state.author_type === 'individual'
                  ? 'bg-[var(--ink)] text-white'
                  : 'bg-white text-[var(--ink-lt)] hover:bg-[rgba(28,43,43,0.04)]'
              }`}
            >
              👤 Individu
            </button>
            <button
              type="button"
              onClick={() => handleAuthorTypeChange('group')}
              className={`flex-1 py-2 text-[12px] font-semibold transition-colors ${
                state.author_type === 'group'
                  ? 'bg-[var(--ink)] text-white'
                  : 'bg-white text-[var(--ink-lt)] hover:bg-[rgba(28,43,43,0.04)]'
              }`}
            >
          ...
          {state.author_type === 'group' ? (
            <div>
              <p className="text-[11px] text-[var(--ink-lt)] mb-2.5 leading-relaxed">
                Pilih kelompok di bagian <strong>Klasifikasi</strong> — semua anggota kelompok otomatis menjadi penulis.
              </p>
              {!state.kelompok_id && (
                <div className="text-center py-3 bg-[var(--cream)] rounded-lg">
                  <p className="text-[11px] text-[var(--ink-lt)] italic">Pilih kelompok terlebih dahulu</p>
                </div>
              )}
              {state.kelompok_id && selectedPenulis.length === 0 && (
                <div className="text-center py-3 bg-[var(--cream)] rounded-lg">
                  <p className="text-[11px] text-[var(--ink-lt)] italic">Memuat anggota kelompok...</p>
                </div>
              )}
              {selectedPenulis.length > 0 && (
                <div className="space-y-1.5">
                  {selectedPenulis.map(p => (
                    <div key={p.id}
                      className="flex items-center gap-2 p-2 bg-[var(--cream)] rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[12px] font-medium text-[var(--ink)] truncate">{p.nama_lengkap}</p>
            ...
            <div>
              <div className="space-y-2 mb-2">
                {selectedPenulis.map(p => (
                  <div key={p.id}
                    className="flex items-center gap-2.5 p-2 bg-[var(--cream)] rounded-lg border border-[rgba(28,43,43,0.07)]">
                    ...
                    <button
                      type="button"
                      onClick={() => updateState('penulis_ids', state.penulis_ids.filter(id => id !== p.id))}
              ...
              {state.penulis_ids.length < 5 && (
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => setShowPenulisPicker(!showPenulisPicker)}
        ...
        <Section title="Klasifikasi">
          <div className="mb-3">
            ...
            <select
              value={state.kategori_id}
              onChange={e => updateState('kategori_id', e.target.value)}
          ...
          <div>
            <label className="block text-[11px] font-semibold text-[var(--ink-lt)] mb-1.5">
              Kelompok Riset
              {state.author_type === 'group' && (
            ...
            <select
              value={state.kelompok_id}
              onChange={e => updateState('kelompok_id', e.target.value)}
              className="w-full border-[1.5px] border-[rgba(28,43,43,0.12)] rounded-lg px-3 py-2 text-[13px] text-[var(--ink)] bg-white outline-none focus:border-[var(--coral)] cursor-pointer font-body">
              <option value="">Pilih kelompok...</option>
              {kelompokList.map(k => (
                <option key={k.id} value={k.id}>
                  Kelompok {k.nomor}: {k.nama}
                </option>
              ))}
            </select>
            {state.author_type === 'group' && !state.kelompok_id && (
              <p className="text-[10px] text-[var(--coral)] mt-1">
                Wajib pilih kelompok untuk artikel kelompok
              </p>
            )}
          </div>
        </Section>
        ...
        <Section title="Kata Kunci">
          <div className="flex flex-wrap gap-1.5 p-2.5 border-[1.5px] border-[rgba(28,43,43,0.12)] rounded-lg min-h-[44px] cursor-text focus-within:border-[var(--coral)] transition-colors">
            {state.kata_kunci.map(kw => (
              <span key={kw}
                className="flex items-center gap-1 px-2.5 py-0.5 bg-[var(--coral-lt)] rounded-full text-[11px] font-semibold text-[#C05030]">
                {kw}
        ...
        <Section title="Gambar Sampul">
          ...
          {state.foto_sampul_url ? (
            <div className="relative rounded-xl overflow-hidden mb-2">
              <Image src={state.foto_sampul_url} alt="Cover" width={280} height={150}
            ...
            <button type="button" onClick={() => coverRef.current?.click()}
              ...
              <p className="text-[12px] text-[var(--ink-lt)]">
                {uploading === 'cover' ? 'Mengupload...' : 'Klik untuk upload gambar'}
              </p>
          ...
          {state.pdf_url ? (
            <div className="flex items-center gap-3 p-3 bg-[var(--cream)] rounded-xl border border-[rgba(28,43,43,0.08)]">
              ...
              <button type="button" onClick={() => pdfRef.current?.click()}
                className="text-[11px] text-[var(--coral)] font-semibold">
                Ganti
              </button>
            </div>
          ) : (
            <button type="button" onClick={() => pdfRef.current?.click()}
              ...
              <p className="text-[12px] text-[var(--ink-lt)]">
                {uploading === 'pdf' ? 'Mengupload...' : 'Klik untuk upload PDF'}
              </p>
        ...
        <Section title="Volume & Edisi" defaultOpen={false}>
          <InputField label="Volume" value={state.volume}
            onChange={e => updateState('volume', e.target.value)} placeholder="Contoh: Vol.1" />
          <InputField label="Nomor Edisi" value={state.nomor_edisi}
            onChange={e => updateState('nomor_edisi', e.target.value)} placeholder="Contoh: No.1" />
        </Section>
