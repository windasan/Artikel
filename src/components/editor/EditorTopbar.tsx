...
export function EditorTopbar({ saveStatus, onSubmit, onSaveDraft, articleId }: Props) {
  const dotColor = { saved:'bg-green-400', saving:'bg-yellow-400 animate-pulse', unsaved:'bg-gray-300', error:'bg-red-400' }[saveStatus]
  const dotLabel = { saved:'Tersimpan', saving:'Menyimpan...', unsaved:'Belum tersimpan', error:'Gagal simpan' }[saveStatus]
    ...
    <div className="h-[52px] border-b border-[rgba(28,43,43,0.08)] flex items-center px-6 gap-4 bg-white flex-shrink-0">
      ...
      <div className="ml-auto flex gap-2">
        ...
        <button onClick={onSubmit}
          ...
          <Send size={14} /> Submit untuk Review
        </button>
      </div>
    </div>
  )
